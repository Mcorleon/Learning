...
public class UserServiceImpl implements UserSerive {
    ...
    public Result userLogin(String id, String psw,HttpServletRequest request) {
        ...
        if (currentUser.isAuthenticated()) {
            ...
            try {
                ...
                redisTemplate.opsForValue().set(sessionID,MAPPER.writeValueAsString(user));
            } catch (JsonProcessingException e) {
                e.printStackTrace();
            }
            redisTemplate.expire(sessionID,60, TimeUnit.MINUTES);
    ...
    public User getCurrentUser() {
        Subject currentUser = SecurityUtils.getSubject();
        String obj=redisTemplate.opsForValue().get(currentUser.getSession().getId().toString());
