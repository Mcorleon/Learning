...
public class deal_VI {
    public static void main(String[] args){
        File file=new File("src/deep_learning_data/run_data1_vi1.csv");
        ...
        try {
            ...
            while (line != null) {
                if(count%10==1||count%10==0){
                    ...
                    float r_x=Float.parseFloat(dara[4]);
                    float r_y=Float.parseFloat(dara[5]);
                    float r_z=Float.parseFloat(dara[6]);
                    float r_w=Float.parseFloat(dara[7]);
            ...
            //true在末尾继续写，false覆盖
            File output=new File("src/deep_learning_output/run_data1_dis_vi.csv");
    ...
    private static double cal_degree(float x, float y, float z, float w) {
        //四元数转欧拉角
        double elur=Math.asin(2*(w*y-x*z));
        //弧度转度数
        return elur*180/Math.PI;
    }
}
